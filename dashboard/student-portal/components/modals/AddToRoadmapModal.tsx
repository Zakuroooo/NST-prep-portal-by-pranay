...
export default function AddToRoadmapModal({ company, onClose, onAdded }: Props) {
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error(err);
      alert("Failed to add company to roadmap");
    } finally {
      setLoading(false);
    }
    setTimeout(() => {
          ...
          {added ? (
            ...
            <>
              ...
              <button
                ...
                className="w-full bg-blue-700 text-white py-3 rounded-xl text-sm font-bold hover:bg-blue-800 transition-colors disabled:opacity-50"
              >
                {loading ? "Adding..." : added ? "Added" : `Add ${company.name} to My Roadmap →`}
              </button>
            </>
          )}
        </div>
